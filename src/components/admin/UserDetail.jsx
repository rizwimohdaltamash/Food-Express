  ...
  if (loading) {
    return (
      ...
      >
        <svg
          aria-hidden="true"
          className="w-16 h-16 mr-2 text-orange-200 animate-spin fill-orange-600"
          viewBox="0 0 100 101"
          fill="none"
          ...
          <table className="w-full text-left border border-collapse sm:border-separate border-orange-400 text-orange-900">
            <tbody>
              <tr>
                {["S.No.", "Name", "Email", "Uid", "Role", "Date"].map(
                  (heading) => (
                    <th
                      key={heading}
                      className="h-12 px-6 text-md border-l first:border-l-0 border-orange-400 text-slate-700 bg-orange-100 font-bold"
                    >
                      {heading}
                    </th>
                  )
              ...
              {getAllUser.map((value, index) => (
                <tr
                  key={index}
                  className="text-orange-900 transition hover:bg-orange-50"
                >
                  <td className="h-12 px-6 text-md border-t border-l first:border-l-0 border-orange-400 text-slate-600">
                    {index + 1}
                  </td>
                  <td className="h-12 px-6 text-md border-t border-l first:border-l-0 border-orange-400 text-slate-600 first-letter:uppercase">
                    {value.name}
                  </td>
                  <td className="h-12 px-6 text-md border-t border-l first:border-l-0 border-orange-400 text-slate-600">
                    {value.email}
